...
def f(nu,  a_0, a_1,a_2):
    return (a_2 / np.sqrt((1 - a_0 * (nu/1000) ** 2) ** 2 + ((nu/1000) ** 2) * a_1))
...
plot.errorbar(x/1000, z + (a / 10), xerr=None, yerr=(a / 10), fmt='rx', label="Kondensatorspannung")

#plot.xscale('log')
plot.xlabel(' $\omega \;in\; \mathrm{ kHz }$')
plot.ylabel('$U_c/U_0$')
plot.xlim(1.6e2, 2.5e2)

#plot.grid()
#plot.legend(loc='best')
plot.tight_layout(pad=0, h_pad=1.08, w_pad=1.08)
#plot.plot(x_plot, f(x_plot, *params), 'b-', linewidth=1)
plot.plot(x_plot1/1000, f(x_plot1, *params), 'b-', label="Fit")
plot.legend(loc='best')
